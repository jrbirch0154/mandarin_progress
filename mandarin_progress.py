...
#%% DF Work
    ...
    df = df[df['Date'] >= '2026-01-02'] # before 1/1/26 I didn't take it seriously
    df['Level'] = pd.cut(df['Total (H)'],
...
def line_plot(df):
    ...
    fig.update_traces(fill='tozeroy',fillcolor='rgba(0,0,0,.1)',line_width=2.5)
    
    fig.update_layout(xaxis_title='Date',
                      yaxis_title='Hours')
    
    for key, value in MILESTONES.items():
        fig.add_hline(y=value,line_dash='dash',line_width=.5,
                      annotation_text=f'Level {key}',annotation_position='top right')
...
def bar_plot(df):
    ...
    fig2.add_hline(y=DAILY_TARGET,line_dash='dash',line_width=.5,line_color='red')
                   #annotation_text='Daily Target (30 min)',annotation_position='top left', annotation_xshift=50, annotation_yshift=100)
    
    fig2.add_trace(go.Scatter(x=df['Date'],y=df['rolling_avg'],
...
def violin_plot(df):
    ...
    fig3.update_layout(title='Input Violin',
                       yaxis_title='Input [M]')
...
def consist_plot(df):
    fig4 = px.line(df,x='Date',y='target_rolling',title='Mandarin Input Consistency')
    ...
    fig4.update_layout(xaxis_title='Date',
                       yaxis_title='30 minute consistency (%)')
    
    overall_avg = df['target_rolling'].mean()
    
    fig4.add_hline(y=overall_avg,annotation_text='Overall Average',annotation_position='top left')
    
    fig4.add_hline(y=(3/7)*100,line_color='red',line_dash='dash',
